...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
	...
	for(int i = 0; i < m; i++)
	{
		...
		switch(r)
		{
			case 0: hosts[i].y = (hosts[i].y + 1) > k ? -k : hosts[i].y + 1;
					break;
			case 1: hosts[i].x = (hosts[i].x + 1) > k ? -k : hosts[i].x + 1;
					break;
			case 2: hosts[i].y = (hosts[i].y - 1) < -k ? k : hosts[i].y - 1;
					break;
			case 3: hosts[i].x = (hosts[i].x - 1) < -k ? k : hosts[i].x - 1;
					break;
		}
